...
begin
  ...
  Pkg.add.(["StatsBase", "DataStructures"])
  Pkg.instantiate()
...
begin
  ...
  const POP_SIZE = 50
  const MAXSTEP = 200
  const STOP = (idle = floor(0.1 * MAXSTEP) |> Int, tol = 1e-4)  # will stop at `step` if gbest[step] and gbest[step-idle+1] close to each other (no improvement in the last `idle` interval)
  function ga0()
    gbest = choose()
    gbest.val = Inf
